...
for epoch in range(500):
    ...
    for iteration in range(Train_num // batchsize):
        ...
        sen_minibatch_index_tensor = 50256*torch.ones([batchsize, maxlength+2], dtype=int).to(device)
        label_minibatch_index_tensor = -100 * torch.ones([batchsize, maxlength + 2], dtype=int).to(device)
        ...
        if (iteration+1) % 100 == 0:
            ...
            if test_loss < best_loss:
                best_loss = test_loss
                print('\n')
                torch.save(model, './trained_model/coco_caption/gpt2_117M_params.pkl')
                not_improved = 0
            else:
                not_improved += 1
