...
if not load_fine_tune_GPT:
    ...
    model = torch.load('./trained_model/coco_caption/gpt2_117M_params.pkl')
model.eval()
...
dim_LSTMh = 768
lm_head = torch.tensor(model.lm_head.weight.data.to('cpu').numpy())
...
## Load data
with open("./dataset/coco_captions/train.json","r") as f:
    data_train = json.loads(f.read())

Test_data = Prepare_test_data("./dataset/coco_captions/test.json")
...
discriminator_optimizer = optim.Adam(discriminator_params, lr=learning_rate_D)
...
for epoch in range(100):
    ...
    for iteration in range(Train_num // batchsize):
        ...
        if (iteration + 1) % 10 == 0:
            print(
                'At epoch %d iteration %d, the training loss is %4f, the reg loss is %4f, the d loss is %4f, the g loss is %4f, the regGAN is %4f, Total lr decay is %d' % (
                    epoch + 1, iteration + 1, loss, reg.item(), dloss.item(), gloss.item(), regGAN.item(), Total_lr_decay))
        ...
        if (iteration + 1) % 100 == 0:
            ...
            with torch.no_grad():
                ...
                for it in range(len(Test_data[0]) // 20):
                    ...
                    Gpt_output = model(test_ids, labels=test_label)
                    gpt_hidden_output = Gpt_output[3]
                    loss_gpt = Gpt_output[0]
                ...
                test_loss2 = test_loss2 / (it + 1)

            if test_loss < best_loss:
                best_loss = test_loss
                print('\n')
                torch.save(LSTM, './trained_model/coco_caption/LSTM_GPT_L2reg_GAN_L3.pkl')
                not_improved = 0
            else:
                not_improved += 1
                ...
                if not_improved == 5:
                    ...
                    learning_rate_D = learning_rate_D * 0.5
                    LSTM_optimizer = optim.Adam(LSTM.parameters(), lr=learning_rate)
                    discriminator_optimizer = optim.Adam(discriminator.parameters(), lr=learning_rate_D)

            print('At epoch %d iteration %d, the validation loss is %4f, the reg loss is %4f, the best loss is %4f, gpt_loss is %4f' % (
            epoch + 1, iteration + 1, test_loss, test_reg, best_loss, test_loss2))
            print('\n')
